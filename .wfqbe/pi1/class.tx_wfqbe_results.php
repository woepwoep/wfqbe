...
class tx_wfqbe_results
{
	...
	$sortField = $this->pibase->piVars['orderby'][$row['uid']]['field'];
	$sortMode = strcmp($this->pibase->piVars['orderby'][$row['uid']]['mode'],'ASC') ? SORT_DESC : SORT_ASC;
	array_multisort(array_column($rows, $sortField), $sortMode, $rows);
	    ...
	    'content' => $content,
            'rows' => $rows,
	    'sortField' => $sortField,
	    'sortMode' => $sortMode == SORT_ASC ? 'DESC' : 'ASC'
        );
