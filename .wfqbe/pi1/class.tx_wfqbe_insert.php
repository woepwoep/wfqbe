...
//$rA['###INSERT_ADD_NEW###'] = "<button type='submit' name='tx_wfqbe_pi1[wfqbe_add_new]' value='".$key."'><img src='".\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path."' /></button>";
    ...
    function showCalendar($value, $name, $id, &$blockTemplate)
    {
            if (!$this->pibase->beMode) {
				...
				// $jsCode .= '<link rel="stylesheet" href="//code.jquery.com/ui/1.12.1/themes/smoothness/jquery-ui.css">';
                // $jsCode.= '<script>
                //            jQuery(document).ready(function() {
                //                $( "#' . $id . '" ).datepicker({"dateFormat": "' . $format . '", "defaultDate": $.datepicker.parseDate("' . $format . '",jQuery("#' . $id . '").attr(\'value\'))});
				//				//jQuery( "#field_crdate_2" ).datepicker();
                                
				//				alert ("' . $id . ': "+jQuery("#' . $id . '").attr(\'value\'));
                //                $( "#' . $id . '" ).datepicker( "option", "dateFormat", "' . $format . '" );
                                
                //                $( "#' . $id . '" ).datepicker( "option", "changeYear", true );
                //                $( "#' . $id . '" ).datepicker( "option", "constrainInput", false );
                //            ';
                // if ($value['form']['min_date'] != '') {
                //    $jsCode .= '$( "#' . $id . '" ).datepicker( "option", "minDate", ' . $value['form']['min_date'] . ' );';
                // }
                // if ($value['form']['max_date'] != '') {
                //    $jsCode .= '$( "#' . $id . '" ).datepicker( "option", "maxDate", ' . $value['form']['max_date'] . ' );';
                // }
