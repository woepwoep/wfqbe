...
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wfqbe') . "pi1/class.tx_wfqbe_insert_fluid.php");
...
class tx_wfqbe_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin
{
    ...
    */
    ...
    function do_sGetFormResult($row, $h)
    {
        if ($row['type'] == "insert") {
            // INSERT
	    if ($this->conf["defLayout"] == 2) {
		$INSERT = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance("tx_wfqbe_insert_fluid");
	    } else {
		$INSERT = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance("tx_wfqbe_insert");
	    }
            $INSERT->main($this->conf, $this->cObj, $this);
