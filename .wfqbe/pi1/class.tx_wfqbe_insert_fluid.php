 ...
 */
class tx_wfqbe_insert_fluid
{

    var $fluidObject = array();
    var $conf;
    var $cObj;
    var $pibase;
	...
	$this->fluidObject['wfqbe_editing_mode'] = $this->pibase->piVars['wfqbe_editing_mode'];
	$this->fluidObject['wfqbe_add_new'] = $this->pibase->piVars['wfqbe_add_new'];
	$this->fluidObject['wfqbe_edit_subrecord'] = $this->pibase->piVars['wfqbe_edit_subrecord'];
	$this->fluidObject['wfqbe_delete_subrecord'] = $this->pibase->piVars['wfqbe_delete_subrecord'];
	$this->fluidObject['wfqbe_submit_insert'] = $this->pibase->piVars['wfqbe_submit_insert'];
	$this->fluidObject['wfqbe_submit_modify'] = $this->pibase->piVars['wfqbe_submit_modify'];
	$this->fluidObject['piVarsOrig'] = $this->pibase->piVars['orig'];
	...
	$this->fluidObject['mode'] = $this->mode;
    }
...
\TYPO3\CMS\Core\Utility\DebugUtility::debug('1');
            ...
            $content .= $this->showPageConfirmation($this->pibase->piVars, $this->blocks, $h, $file, $row);
            //RW $content = $this->cObj->substituteSubpart($content, '###MODIFY_BUTTON###', '', 0, 0);


            // Show the insert form
\TYPO3\CMS\Core\Utility\DebugUtility::debug('2');
...
\TYPO3\CMS\Core\Utility\DebugUtility::debug('2a');
            } else {
\TYPO3\CMS\Core\Utility\DebugUtility::debug('2b');
		}
            $file = $this->cObj->getSubpart($file, '###INSERT_TEMPLATE###');
            if (is_array($this->blocks)) {
                $content = $this->showInsertModule($file, $this->blocks['fields'], $h, $row);
            }
        $view = $this->fluidLayout();
        $variables = array(
                'blocks' => $this->blocks,
		'conf' => $this->conf,
		'fluidObject' => $this->fluidObject,
		'row' => $this->row,
        );
...
// dit is de update confirm
\TYPO3\CMS\Core\Utility\DebugUtility::debug('3');
...
$queryUid = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP('queryUid');
$showPage = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP('showPage');
if ($queryUid && $showPage) $params['additionalParams'] .= '&tx_wfqbe_pi1[showpage]['.$queryUid.']='.$showPage;
                        $action = $this->cObj->typoLink_URL($params);
//\TYPO3\CMS\Core\Utility\DebugUtility::debug($action);exit(1);
                        header('Location: ' . \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL') . $action);
                    } else {
            ...
            // Show the confirmation page
\TYPO3\CMS\Core\Utility\DebugUtility::debug('4');
            ...
            // Show the insert form
\TYPO3\CMS\Core\Utility\DebugUtility::debug('5');
            if ($this->pibase->piVars['wfqbe_add_new'] == '' && $this->pibase->piVars['orig']['wfqbe_add_new'] != '') {
                $this->pibase->piVars = $this->pibase->piVars['orig'];
                unset($this->pibase->piVars['wfqbe_add_new']);
                $this->main($this->conf, $this->cObj, $this->pibase);
            }
\TYPO3\CMS\Core\Utility\DebugUtility::debug('6');
            $file = $this->cObj->getSubpart($file, '###INSERT_TEMPLATE###');
            if (is_array($this->blocks)) {
                $content = $this->showInsertModule($file, $this->blocks['fields'], $h, $row);
            }
    ...
    {
\TYPO3\CMS\Core\Utility\DebugUtility::debug(showInsertModule);
	...
	$idField = $this->blocks['ID_field'];
	if ($idField && is_array($editing_record)) $this->fluidObject[$idField] = $editing_record[$idField];
	...
	$this->fluidObject[$fieldName] = array();
	$this->fluidObject[$fieldName]['keyparm'] = $key;
	$this->fluidObject[$fieldName]['blockparm'] = $this->blocks['fields'][$key];
	$this->fluidObject[$fieldName]['checked'] = '';
        $blockList = "";
	...
	// RW  $this->fluidObject[$fieldName]['label'] = $rA['###INSERT_LABEL###'];
	...
	$this->fluidObject[$fieldName]['name'] = 'tx_wfqbe_pi1['.$key.'][]';
        switch ($value['type']) {
            case 'display only':
            ...
            case 'datepicker':
		$this->fluidObject[$fieldName]['name'] = 'tx_wfqbe_pi1['.$key.']';
                $rA['###INSERT_FIELD###'] = $this->showDate($value, $key, $name);
                break;
            case 'timepicker':
		$this->fluidObject[$fieldName]['name'] = 'tx_wfqbe_pi1['.$key.']';
                $rA['###INSERT_FIELD###'] = $this->showTime($value, $key, $name);
                break;
            case 'password':
...
//$rA['###INSERT_ADD_NEW###'] = "<button type='submit' name='tx_wfqbe_pi1[wfqbe_add_new]' value='".$key."'><img src='".\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path."' /></button>";
    ...
    {
	$inputField = new InputField($value, $name, $id);
	...
	$this->fluidObject[$fieldName]['value'] = $this->charToEntity($fieldValue);
	...
	$fieldName = $value['field'];
        ...
        if ($value['form']['source'] == 'static') {
		    ...
		    $this->fluidObject[$fieldName]['value'] = $value['form'][$i]['value'];
		    $this->fluidObject[$fieldName]['checked'] = 'checked';
		    ...
		    $this->fluidObject[$fieldName]['value'] = $array[1];
		    $this->fluidObject[$fieldName]['checked'] = 'checked';
                } else {
	...
	$this->fluidObject[$fieldName]['id'] = $id;
	$this->fluidObject[$fieldName]['update'] = $update;
	    ...
	    $this->fluidObject[$fieldName]['size'] = $size;
	    $this->fluidObject[$fieldName]['multiple'] = 'multiple';
        } else {
            $listaSelect = '<select' . $update . ' id="' . $id . '" name="tx_wfqbe_pi1[' . $name . '][]">';
	    $this->fluidObject[$fieldName]['size'] = '';
	    $this->fluidObject[$fieldName]['multiple'] = '';
	}
	    ...
	    $selectList[] = array(
		'value' => '',
		'label' => $value['form']['labelEmptyValue'],
	    );
	}
		...
		$selectList[] = array(
			'value' => $value['form'][$i]['value'],
			'label' => $value['form'][$i]['label'],
		);
            }
        } else {
            // Source == db
	...
	$this->fluidObject[$fieldName]['selectList'] = $selectList;
...
\TYPO3\CMS\Core\Utility\DebugUtility::debug('showPageConfirmation');
        return $content;
    }
                    ...
                    if ($config['form']['when'][$this->mode] == 1) {
                        ...
                        elseif ($config['type'] == 'PHP function') {
                            ...
                            if ($blocks['fields'][$key]['type']=="datepicker" && $val!="")  {
                                $val = $this->date2unixtime($val);
                            }
                            if ($blocks['fields'][$key]['type']=="timepicker" && $val!="")  {
                                $val = $this->time2unixtime($val);
                            }
        ...
        foreach ($this->blocks['fields'] as $key => $config) {
            if ($this->mode == "delete" || $config['form']['when'][$this->mode] == 1) {
                if ($config['type'] == 'select' || $config['type'] == 'checkbox' || $config['type'] == 'upload') {
                    ...
                    $this->pibase->piVars[$key] = $this->unixtime2date($row[$config['field']]);
                } elseif ($config['type'] == 'timepicker') {
                    $this->pibase->piVars[$key] = $this->unixtime2time($row[$config['field']]);
                } else {
	...
	function showDate($value, $name, $id)	{
		...
		$this->fluidObject[$fieldName]['value'] = $fieldValue;
	...
	function showTime($value, $name, $id)	{
		...
		$fieldName = $value['field'];
                ...
                $GLOBALS['TSFE']->additionalHeaderData[$id] = '
		<script src="fileadmin/JavaScript/jquery.timepicker.js" type="text/javascript"></script>
		<link rel="stylesheet" href="fileadmin/JavaScript/jquery.timepicker.css"> 
		<script>
			$(function() {
    ...
    /**
     * This function is used to create the default results template as fluid template
     */
	...
	// $view->getRequest()->setControllerExtensionName('wfqbe');
